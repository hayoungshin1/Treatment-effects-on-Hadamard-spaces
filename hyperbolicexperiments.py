...
# classical randomized experiment-consistency (don't think this is needed in light of the strong consistency theorem)
...
for j in range(4):
    ...
    for k in range(1000):
        ...
        if np.squeeze(np.sqrt(np.sum((trueans-ans)**2)))<=np.sort(np.squeeze(np.sqrt(np.sum((bootans-ans)**2,2))))[int(0.95*B)]:
            OSexpcounter[j]+=1
        if k % 10==0:
            print(N, k, OSexpcounter[j])
    print(OSexpcounter[j])

print(OSexpcounter)

#np.save('Documents/OSexpcounter.npy',OSexpcounter)
